...
class OPV2VBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True, select_scenario=None):
        ...
        if select_scenario is None:
            ...
            if 'm1_mask' in self.params:
                for scenario in scenario_folders:
                    scenario_name = scenario.split("/")[-1]
                    if scenario_name not in m1_scenario_mask:
                        print("skip:", scenario)
                        scenario_folders.remove(scenario)
        else:
            scenario_folders = [select_scenario]
            print("select scenario:", scenario_folders)
    ...
    def reinitialize(self):
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            if self.train:
                ...
                if 'umask_cav' in self.params:
                    cav_list = sorted(cav_list)
                    print("cav umask skip", cav_list[0])
                    cav_list = cav_list[1:]
                # cav_list = sorted(cav_list)
                ...
                for timestamp in timestamps:
                   ...
                   # load extra data
                    for file_extension in self.add_data_extension:
                        file_name = \
                            os.path.join(cav_path.replace("OPV2V", "OPV2V_Hetero"),
    ...
    def get_scenario_name(self):
        assert len(self.scenario_folders) == 1
        return self.scenario_folders[0].split("/")[-1]